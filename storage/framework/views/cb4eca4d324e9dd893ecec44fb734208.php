<div class="main-nav">
    <!-- Sidebar Logo -->
    <div class="logo-box">
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="logo-dark">
            <img src="<?php echo e(asset('admin/images/logo-sm.png')); ?>" class="logo-sm" alt="logo sm">
            <img src="<?php echo e(asset('admin/images/logo-dark.png')); ?>" class="logo-lg" alt="logo dark">
        </a>
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="logo-light">
            <img src="<?php echo e(asset('admin/images/logo-sm.png')); ?>" class="logo-sm" alt="logo sm">
            <img src="<?php echo e(asset('admin/images/logo-light.png')); ?>" class="logo-lg" alt="logo light">
        </a>
    </div>

    <!-- Menu Toggle Button -->
    <button type="button" class="button-sm-hover" aria-label="Show Full Sidebar">
        <iconify-icon icon="solar:double-alt-arrow-right-bold-duotone" class="button-sm-hover-icon"></iconify-icon>
    </button>

    <div class="scrollbar" data-simplebar>
        <ul class="navbar-nav" id="navbar-nav">

            <li class="nav-item">
                <a class="nav-link" href="<?php echo e(route('admin.dashboard')); ?>">
                    <span class="nav-icon"><iconify-icon icon="solar:widget-5-bold-duotone"></iconify-icon></span>
                    <span class="nav-text"> Tổng quan </span>
                </a>
            </li>

            <!-- Sản phẩm -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#1" data-bs-toggle="collapse" role="button"
                    aria-expanded="false" aria-controls="1">
                    <span class="nav-icon"><iconify-icon icon="mdi:shopping-outline"></iconify-icon></span>
                    <span class="nav-text"> Quản lý sản phẩm </span>
                </a>
                <div class="collapse" id="1">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Danh sách sản phẩm</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="<?php echo e(route('admin.products.categories.index')); ?>">Danh mục</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="<?php echo e(route('admin.brands.index')); ?>">Thương hiệu</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Thuộc tính</a></li>
                    </ul>
                </div>
            </li>

            <!-- Người dùng -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#2" data-bs-toggle="collapse">
                    <span class="nav-icon"><iconify-icon icon="mdi:account-group-outline"></iconify-icon></span>
                    <span class="nav-text"> Quản lý người dùng </span>
                </a>
                <div class="collapse" id="2">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Người dùng</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Nhân viên</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Quản trị viên</a></li>
                    </ul>
                </div>
            </li>

            <!-- Đơn hàng -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#3" data-bs-toggle="collapse">
                    <span class="nav-icon"><iconify-icon icon="mdi:cart-outline"></iconify-icon></span>
                    <span class="nav-text"> Quản lý đơn hàng </span>
                </a>
                <div class="collapse" id="3">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Đơn hàng</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Hủy/Đổi trả</a></li>
                    </ul>
                </div>
            </li>

            <!-- Bài viết -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#4" data-bs-toggle="collapse">
                    <span class="nav-icon"><iconify-icon icon="mdi:post-outline"></iconify-icon></span>
                    <span class="nav-text"> Quản lý bài viết </span>
                </a>
                <div class="collapse" id="4">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Bài viết</a></li>
                    </ul>
                </div>
            </li>

            <!-- Đánh giá -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#5" data-bs-toggle="collapse">
                    <span class="nav-icon"><iconify-icon icon="mdi:star-outline"></iconify-icon></span>
                    <span class="nav-text"> Quản lý đánh giá </span>
                </a>
                <div class="collapse" id="5">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Bình luận</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Phản hồi</a></li>
                    </ul>
                </div>
            </li>

            <!-- Khuyến mãi -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#6" data-bs-toggle="collapse">
                    <span class="nav-icon"><iconify-icon icon="mdi:tag-multiple-outline"></iconify-icon></span>
                    <span class="nav-text"> Khuyến mãi </span>
                </a>
                <div class="collapse" id="6">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Mã giảm giá</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Chương trình</a></li>
                    </ul>
                </div>
            </li>

            <!-- Hệ thống -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#7" data-bs-toggle="collapse">
                    <span class="nav-icon"><iconify-icon icon="mdi:cog-outline"></iconify-icon></span>
                    <span class="nav-text"> Cấu hình hệ thống </span>
                </a>
                <div class="collapse" id="7">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Quản lý banner</a></li>
                    </ul>
                </div>
            </li>

            <!-- Phân quyền -->
            <li class="nav-item">
                <a class="nav-link menu-arrow" href="#8" data-bs-toggle="collapse">
                    <span class="nav-icon"><iconify-icon icon="mdi:shield-key-outline"></iconify-icon></span>
                    <span class="nav-text"> Phân quyền </span>
                </a>
                <div class="collapse" id="8">
                    <ul class="nav sub-navbar-nav">
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Vai trò</a></li>
                        <li class="sub-nav-item"><a class="sub-nav-link" href="#">Phân quyền chi tiết</a>
                        </li>
                    </ul>
                </div>
            </li>
        </ul>
    </div>
</div>
<?php /**PATH C:\laragon\www\TechViCom_Website_BE\resources\views/admin/layouts/sidebar.blade.php ENDPATH**/ ?>